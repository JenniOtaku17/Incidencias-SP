            ...
            <br>
              ...
              <v-row>
                <div class="flex-center  py-5">
                  <v-btn color="primary" elevation="0" class="ml-2" :disabled="invalid" @click="guardar">Guardar Cambios</v-btn>
                </div>
              </v-row>
            </validation-observer>
          </v-container>
  ...
  methods: {
      ...
      async getAll(){
        try{
          ...
          let usuario = await axios.get(this.host+"api/Usuario/Get/"+ id, { 'headers': { 'Authorization': `Bearer ${token}` } });
