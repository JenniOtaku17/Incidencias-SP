                    ...
                    <v-row>
                        <v-col cols="12" sm="12" class="pb-0">
                            <validation-provider
                                ...
                                rules="required"
                            >
                                <v-text-field prepend-icon="mdi-rename-box" :error-messages="errors" dense outlined label="Descripción" v-model="form.Descripcion"></v-text-field>
                            </validation-provider>
                        </v-col>
                        <v-col cols="12" sm="12" class="pb-0">
                            <validation-provider
                                ...
                                rules="required"
                            >
                                <v-text-field prepend-icon="mdi-clock-outline" type="number" :error-messages="errors" dense outlined label="Cantidad de Horas" v-model="form.CantidadHoras"></v-text-field>
                            </validation-provider>
                        </v-col>
                    </v-row>
                    <v-row class="mt-5">
        ...
        toEdit : function (val) {
            if(val){
                this.form = {
                    ...
                    CantidadHoras: val.cantidadHoras,
                    Estatus: val.estatus,
                    CreadoPor: val.creadoPor
                }
                this.editing = true;
                this.title = 'Editar SLA';
                this.dialog = true;
            }
        }
    },
        ...
        async guardar(){
            ...
            try{
                ...
                if(this.editing == true){
                    ...
                    result = await axios.put(this.host+"api/Sla/Update", this.form , { 'headers': { 'Authorization': `Bearer ${token}` } });

                }else{
                    let token = this.user.token;
                    this.form.CreadoPor = this.user.usuarioId;

                    result = await axios.post(this.host+"api/Sla/Add", this.form , { 'headers': { 'Authorization': `Bearer ${token}` } });
                }
